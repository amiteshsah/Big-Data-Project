...
for line in sys.stdin:
	if 'unemployment' in os.environ.get('mapreduce_map_input_file'):
		...
		# if NTA_code_temp not in NTA_code_list:
		# 	NTA_code_list.append(NTA_code_temp)
		NBD_name=data[i+2]
		year=data[i+3]
		unemp=data[i+4]
		population=data[i+5]
		#crime_rate=data[i+6]
		crime_rate='NULL'
		#print'%s\t%s' %(str(NTA_code)+' ,'+str(year),str(Borough_name)+' ,'+str(NTA_code)+' ,'+str(NBD_name)+' ,'+str(year)+' ,'+str(unemp)+' ,'+str(population)+' ,'+str(crime_rate))
		print'%s\t%s' %(str(NTA_code)+' ,'+str(year),str([Borough_name,NTA_code,NBD_name,year,unemp,population,crime_rate]))

	if 'crimerate' in os.environ.get('mapreduce_map_input_file'):
		data=list(csv.reader([line],delimiter=','))[0]
		# if firstline:    #skip first line
		# 	firstline = False
		# 	continue
		...
		for temp_NTA_code in NTA_code_list:
			if NTA_code in temp_NTA_code:
				...
				print'%s\t%s' %(str(NTA_code)+' ,'+str(year),str(['NULL',NTA_code,'NULL',year,'NULL','NULL',count]))
				#NTA_code=','.join(temp_NTA_code)
				break
		...
		#print'%s\t%s' %(str(NTA_code)+' ,'+str(year),'NULL'+' ,'+str(NTA_code)+' ,'+'NULL'+' ,'+str(year)+' ,'+'NULL'+' ,'+'NULL'+' ,'+str(count))
		#print'%s\t%s' %(str(NTA_code)+' ,'+str(year),str(['NULL',NTA_code,'NULL',year,'NULL','NULL',count]))
